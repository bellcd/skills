...
They apply to new code and to code already being edited. Sweep pre-existing violations in the file being touched, and leave the rest alone. A standalone bulk refactor to satisfy one of these rules is not wanted unless asked for, and older code that predates a rule is not a pattern to copy.
